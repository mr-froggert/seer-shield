...
import { formatChain, formatDate, formatPercent } from "../lib/format";
...
import {
  getFilteredLinkedRoutes,
  getHighestLinkedApyRoute,
  getLinkedApyRange
} from "../lib/yieldRoutes";
import type { CoreAssetOverviewViewModel } from "../lib/types";

function getRealMarkets(asset: CoreAssetOverviewViewModel["linkedMarkets"]) {
  return asset.filter((market) => market.signal.marketSyncStatus === "created" && Boolean(market.signal.url));
}

export function CoreAssetDetailPage() {
  const { riskAdjustedApy } = useAppPreferences();
  const { id } = useParams();
  const navigate = useNavigate();
  const { coreAssets, protocols } = useTerminalData();
  const [selectedChainIds, setSelectedChainIds] = useState<number[]>([]);
  const [sortBy, setSortBy] = useState<YieldAssetProtocolSortBy>("safety");
  const asset = coreAssets.find((entry) => entry.id === id);

  if (!asset) {
    return (
      <main className="page-shell">
        <div className="callout error-callout">
          Core asset not found. <Link to="/core-assets">Return to the core assets table.</Link>
        </div>
      </main>
    );
  }
        ...
        <div className="intro-copy">
          <div className="intro-heading">
            ...
            <div className="intro-title-block">
              <p className="section-kicker">Core asset</p>
              <h1 className="page-title">{asset.symbol}</h1>
            </div>
          </div>
          <p className="hero-copy">{asset.description ?? `${asset.name} is tracked as a base asset across the configured yield routes.`}</p>
          <ProtocolBadgeGroup items={asset.tags} className="detail-badge-row" />
          <div className="detail-actions">
            <a className="inline-link inline-link-strong" href={asset.website ?? asset.protocol.website} target="_blank" rel="noreferrer">
              Website
            </a>
            {defillamaUrl ? (
        ...
        <div className="summary-grid">
          ...
          <div className="summary-card">
            ...
            {topYieldRoute ? (
              ...
              <span className="table-subline">No tracked opportunity for this asset</span>
            )}
          </div>
          <div className="summary-card summary-card-empty" aria-hidden="true" />
          <div className="summary-card">
      ...
      <section className="surface-panel detail-support-panel detail-table-panel">
        <div className="card-header">
          ...
          <div className="table-controls">
            <div className="field field-inline">
              <label className="control-label" htmlFor="core-asset-detail-sort">
                Sort by
              </label>
              <select
                id="core-asset-detail-sort"
                value={sortBy}
        ...
        {protocolRows.length === 0 ? (
          <div className="callout subtle-callout">
            {selectedChainIds.length === 0
              ? "No tracked yield route for this asset yet."
              : "No tracked yield route on the selected chains."}
          </div>
        ) : (
